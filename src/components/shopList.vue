          ...
          <div class="item-list-wrap">
            <div class="cart-item">
              ...
              <ul class="cart-item-list">
                <li v-for="item in shoppingList" :key="item.productId">
                  ...
                  <div class="cart-tab-2">
                    <div class="item-price">{{ item.productPrice | moneyFloat }}</div>
                  </div>
                  ...
                  <div class="cart-tab-4">
                    <div class="item-price-total">{{item.productQuantity*item.productPrice | moneyFloat}}</div>
                  </div>
                  <div class="cart-tab-5">
                    <div class="cart-item-operation">
                      <a href="address" class="item-edit-btn" @click="delConfirm(item)">
          ...
          <div class="cart-foot-wrap">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascript:void 0">
                  <span class="item-check-btn" :class="{'check': checkAll}" @click="selectAll(true)">
                    <svg class="icon icon-ok"><use xlink:href="#icon-ok"></use></svg>
                  </span>
                  <span>全选</span>
                </a>
              ...
              <div class="item-all-del">
                <a href="javascript:void 0" class="item-del-btn" :class="{'check': !checkAll}"  @click="selectAll(false)">
                  <span>取消全选</span>
                </a>
              </div>
            </div>
            <div class="cart-foot-r">
              <div class="item-total">
                Item total: <span class="total-price">{{productTotalMoney | moneyFloat}}</span>
              </div>
...
export default {
  ...
  methods: {
    fetchData: function () {
      var self = this;
      this.$http.get("./src/assets/data/cartData.json").then((res)=>{
        self.shoppingList = res.data.result.list;
      })
    },
    changeQuantity: function(product, calcNum){
      product.productQuantity += calcNum;
    ...
    calcTotalMoney: function(){
      ...
      this.shoppingList.forEach(function(item, index) {
        if(item.checked){
          console.log(self.productTotalMoney);
          self.productTotalMoney += (item.productPrice*item.productQuantity);
        }
      });
    },
    selectAll: function (bool) {
      var self = this;
      this.checkAll = bool;
      this.shoppingList.forEach(function(value, index) {
        value.checked = bool;
      })
      this.calcTotalMoney()
    },
    ...
    delProduct:function(){
      var idx = this.shoppingList.indexOf(this.curProduct);
